...
- `apps/web-viewer/src/studio-replay-utils.ts`
  - Pure replay helper layer (cinematic plan, coast decay math, path metrics, run-path/coast-end safety mapping, collision/separation policy, transient-state reset, label-application policy, camera selection, and ranking helper functions).
